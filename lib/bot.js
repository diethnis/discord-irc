...
class Bot {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        if (text !== '') {
          ...
          var cmdsymb = '`!#$%^&*,./?:;\'\"';
          if ( cmdsymb.indexOf(text[0]) > -1 ) {
            //text = Bot.substitutePattern(this.format.ircText || '{$text}', patternMap);
          } else {
  ...
  sendToDiscord(author, channel, text) {
    ...
    var cmdsymb = '`!#$%^&*,./?:;\'\"';
    if ( cmdsymb.indexOf(text[0]) > -1 ) {
      logger.debug('Sending message to Discord', text, channel, '->', `#${discordChannel.name}`);
      discordChannel.send(text);
    } else {
